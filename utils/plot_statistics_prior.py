...
def load_and_plot_statistics(pkl_path, save_dir):
    ...
    # 绘制三个类别的标准差范数
    for i in range(3):
        plt.plot(epochs, stats['sigmas_norm'][f'class_{i}'], 
                label=f'Class {i} Std Norm', 
                linestyle='-')
    
    # 计算并绘制类间距差异
    nablas_diff_01 = np.array(stats['nablas_norm']['class_1'])
    nablas_diff_12 = np.array(stats['nablas_norm']['class_2'])
    ...
    # 保存图表
    save_path = os.path.join(save_dir, 'detailed_statistics_prior.png')
